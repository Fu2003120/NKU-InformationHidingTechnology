...
% 定义加密密钥
encryptionKey = randi([0, 1], size(watermarkImage));

% 加密水印图像
encryptedWatermarkImage = xor(watermarkImage, encryptionKey);
...
for i = 1:numBlocks
    for j = 1:numBlocks
        ...
        % 根据加密后的水印图像调整DC系数
        modFactor = (1 + 0.01 * (2 * encryptedWatermarkImage(i, j) - 1));
...
% 提取图像水印
extractedImage = zeros(size(encryptedWatermarkImage));
for i = 1:numBlocks
    for j = 1:numBlocks
        xStart = (i - 1) * blockSize + 1;
        yStart = (j - 1) * blockSize + 1;
...
% 解密水印图像
decryptedExtractedImage = xor(extractedImage, encryptionKey);
...
subplot(235); imshow(decryptedExtractedImage, []); title('提取图像');
subplot(236); imshow(imcomplement(decryptedExtractedImage), []); title('提取图像后反色与原图对比');
